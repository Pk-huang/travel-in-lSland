...
## 2026-08-31 目前執行重點：完成 Phase 2-2 測試、架構重整優先

- 本輪目標：完成渲染初始化層級的完整測試覆蓋（179 tests），推進架構重整確保後續開發穩定性。
- 完成現況（2026-08-31）：Phase 2-2 七大子階段測試全過、資料轉換層測試完成、面板與互動邏輯重構完成。
- 本項目已提升為 Phase 3 中的最優先選項，優先於其他展示優化與功能擴充。
- 重整重點：
  1. 以領域為單位整理元件與資料夾，避免功能邏輯散落於不同層級。
  2. 釐清 page / providers / store / map / panel 的職責與資料流。
  3. 把旅行計畫、POI、地圖互動的共用邏輯收斂，避免跨層重複實作。
  4. 建立後續開發的架構規則，包含 state、data、UI 的分層與邊界。
- 暫緩項目：完整 E2E 測試、大規模 AI 能力擴張、LOD / 地形架構大改。
- 交付期待：專案能讓新成員快速理解，主要功能入口清楚，後續功能開發能建立在穩定基礎上。
...
### Phase 3 執行補充（2026-07-20 調整）

- 調整背景：目前目標從「可上線」提升為「可穩定 Demo 且功能完整度更高」。
- 執行策略：採雙軌並行，`功能優先（60%） + 最小防線測試（40%）`。
- 取捨原則：不砍掉全部測試，改為保留最小防翻車測試；深度壓測與完整 E2E 可延後。

### 計畫項目 3-4：一週功能優先衝刺（Demo Ready Sprint）

- 狀態：主線新增（功能優先，測試降載不失守）
- 目標：在 1 週內完成「景點與照片展示 + 旅行清單 CRUD + 輕量 AI 建議」的可演示閉環。
- 解決方案：
  1. POI 展示：景點清單、詳情卡、照片載入與 fallback。
  2. 旅行清單：local-first CRUD（新增/編輯/刪除/完成），並支援從 POI 一鍵加入。
  3. 輕量 AI：依偏好生成 3~5 筆建議，支援一鍵加入清單（避免重複）。
  4. 清單驅動優化：依清單內容調整推薦排序與風險提示。
- 最小測試底線（全程跟著功能走）：
  1. `get_errors`（型別/編譯）
  2. `corepack pnpm lint`
  3. 首頁健康檢查（home 200）
  4. 核心 smoke：API fallback 不白屏、CRUD 基本流程可走通
- 一週執行節奏（Day 1~Day 7）：
  1. Day 1-2：POI 與照片展示完成（含錯圖 fallback）
  2. Day 3-4：旅行清單 CRUD + 與 POI 串接
  3. Day 5：輕量 AI 建議 + 一鍵回填清單
  4. Day 6：清單驅動優化 + UI 狀態一致化（loading/error/empty/fallback）
  5. Day 7：彩排與修補（連續 Demo 驗收）
- 驗收標準：
  1. 使用者可看景點資料與照片。
  2. 使用者可完整操作旅行清單 CRUD。
...
#### 3-4 UI / 地圖先行策略（2026-07-24 決議）
- 決策主軸：先完成「可見、可點、可導引」的 UI 與地圖閉環，再做高風險智慧化能力。
- 執行順序（最小步）：
  1. 地圖站點標記顯示行程順序（第幾站）。
  2. 左側單日行程點擊可導引 Google Maps（優先座標，無座標 fallback 地名）。
  3. 左側切換「全部行程 / 單日行程」。
  4. 單日行程編輯能力（拖曳排序、新增、刪除、欄位編輯）。
  5. 全部行程層級編輯能力（跨日拖曳排序、新增、刪除）。
  6. 顯示使用者當前定位（需瀏覽器權限）。
- Google Maps 連結生成規則：
  - 單站導引：`/maps/dir/?api=1&destination=`，有 `lat/lon` 用座標，否則用地名。
  - 單日路線：`origin + destination + waypoints`，超長時裁切 waypoint。
  - 無可用站點時：禁用導引按鈕並給 UI 提示。
- 保留方案（本輪不實作）：
  - 激進實驗：依天數/開車時數/景點類型自動重排（Auto Replan）。
  - 保留原因：先確保基礎編輯與地圖互動可靠，避免同時引入高不確定性。
...
調整註記（2026-07-20）：
- 目前採 `Phase 3-4 一週功能優先衝刺` 先完成 Demo Ready 閉環（POI/照片 + 清單 CRUD + 輕量 AI）。
- `3-2 深度壓測`、`3-1 完整測試覆蓋`、`3-3 完整上線防線` 維持在衝刺後補齊，不取消。
...
## 6. 立即執行清單（Next 5 Actions）— 2026-08-31 更新

### ✅ 已完成（2026-08-31）
1. ✅ Phase 2-2 Rendering Initialization Testing：179 tests 全過
   - 2-2a ~ 2-2g 七大子階段（相機、場景、Canvas、光影、地形、渲染器、材質）
   - 效能預算驗證：256→2-3MB、512→9MB、1080→40MB
   - Commit: e27275d

2. ✅ 資料轉換層測試：79 tests 全過
   - CRS 座標轉換、顏色映射、網格生成三層
   - 單元與整合測試完整覆蓋
   - Commit: 2f64e0a

3. ✅ 面板與互動邏輯重構：Marker Interaction Core 分離、Panel 模組扁平化、Timeline/Scene 控制拆分
   - 架構責任邊界更清晰

### 🔄 進行中（2026-08-31 ~ 2026-09-07）
1. 架構重整（主線）：元件與資料夾領域整理、責任邊界釐清、共用邏輯收斂
2. 預期完成：2026-09-07 前

### ⏸️ 暫緩（待架構重整驗收）
1. Phase 3-4 Day 3 以後：輕量 AI 建議、清單驅動優化
   - 優先完成架構重整，確保後續功能開發基礎穩定

### 📋 下一步執行順序
1. Phase 2-2g 實作（2026-09-04 ~ 09-07）：Material & Geometry 整合驗證
2. 效能基線量測（2026-09-07 ~ 09-10）：512 DEM 固定解析度 FPS/記憶體對比
3. 最小防翻車測試（2026-09-10 ~ 09-14）：smoke test + API fallback 驗證
4. 每日保留最小測試底線：`get_errors` + `corepack pnpm lint` + home 200
5. 架構重整驗收後決議 Phase 3-4 功能衝刺重啟時程
