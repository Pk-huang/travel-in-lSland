...
## Phase 2：3D 地圖與互動 UI（主線 7~10 天；含選配 10~15 天）

> 執行順序備註（2026-07-09）：2-3「時間軸與 API 一致性」主線已完成；2-4 行動端手勢隔離暫緩（目前以桌機體驗為主）。
> Phase 2 選配強化採固定順序：C-1 → B → A → C-2。
...
### Phase 2 選配強化（重寫策略與執行順序）

- 策略：先做能快速拉開展示差異、且可直接驗證決策價值的項目；採「先做到可觀察極限，再回頭修整細節」方式推進。
- 執行順序：C-1 → B → A → C-2
- 銜接規則：選配強化完成後，立即進入 4-2 效能與壓力測試，先完成整體負載驗證再往下一階段。
...
- 建議：作為第一個選配落地，完成後先做 before/after 視覺與效能記錄

#### 選配 B：LOD 強化
- 目標：放大場景時保留地形細節並控制效能
- 方案：
  1. LOD MVP（chunked LOD 基礎層級切換）：2.5~3 天
  2. LOD 強化（更高層級細節/切換體驗優化）：再加 1.5~2 天
- 建議：C-1 完成後立刻接續，建立可量測的地形細節提升幅度
...
- 建議：放在 B 後面，避免過早引入視覺噪音干擾地形/光影驗證
...
- 建議：待 C-1、B、A 都完成後再做，確保收斂基準穩定

---

## Phase 3：測試、上線與營運（3~5 天）

> 時機說明：Phase 2 主線與選配強化（C-1 → B → A → C-2）完成後，先執行 3-2 壓測建立最終效能基線，再完成其餘測試/上線防線，最後進入 AI 功能擴張。
...
- 解決方案：
  ...
  2. 模擬高頻資料更新（例如每 10 秒）
  3. 針對低階手機做 FPS/記憶體量測
  4. LOD / 雲層 / 光影的 before/after 對比量測（FPS、記憶體、互動延遲）
- 交付物：效能報告與優化清單
...
- Week 2~3（已完成主線）：Phase 2 主線（2-3 時間軸與 API 一致性已完成；2-4 行動端手勢隔離暫緩）
- Week 4：Phase 2 選配強化（C-1 → B → A → C-2）+ Phase 3-2 效能壓測
- Week 5：Phase 3（3-1 測試策略 + 3-3 CI/CD 上線防線）
...
4. 選配 C-1：完成光影平滑過渡 + preset URL 同步，並產出 before/after 對比
5. 依序完成 B → A → C-2，完成後立即執行 3-2 壓測（含 LOD/雲層/光影 before/after）
