...
## 2026-08-01 目前執行重點：Phase 3 中最優先的選項
- 本輪目標不是再追加大功能，而是先把現有功能的結構與責任邊界整理完成，讓後續增刪改更安全。
...
1. 啟動 3-4 Day 1-2：完成 POI 資料與照片展示（含錯圖 fallback）
2. 啟動 3-4 Day 3-4：完成旅行清單 CRUD（新增/編輯/刪除/完成）
3. 啟動 3-4 Day 5：導入輕量 AI 建議並支援一鍵加入清單
4. 啟動 3-4 Day 6：依清單內容做推薦排序與風險提示優化
5. 啟動 3-4 Day 7：進行連續 Demo 彩排與缺陷修補
6. 每日保留最小測試底線：`get_errors` + `corepack pnpm lint` + home 200
7. 3-4 完成後回補 3-2 深度壓測（固定 512 基線）
8. 3-4 完成後回補 3-1/3-3 完整測試與上線防線
