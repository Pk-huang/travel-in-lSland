...
> 執行順序調整（2026-07-07）：先完成 2-3「時間軸與 API 一致性」主線，再進 2-4 行動端手勢隔離。
...
#### 選配 C：光影增強
- 目標：提升光影切換體感、可分享性與面試可解釋性
- 方案：
  1. preset 切換平滑過渡（光強/色彩內插）：0.5~1 天
  2. preset URL 同步（可分享連結）+ reset to realistic：0.5~1 天
  3. seasonal 日照長短映射（夏長日/冬短日）+ 當前 preset 標示：1~2 天
  4. `computeLighting` 單元測試 + internal override 僅 dev 生效：0.5~1 天
- 建議：若以面試展示為導向，優先做「平滑過渡 + URL 同步」；seasonal 映射作為第二階段加分
...
- Week 2~3：Phase 2 主線（先 2-3 時間軸與 API 一致性，再 2-4 行動端手勢）
