...
本專案的核心目標是**累積大量實戰經驗**，刻意走非保險路線才能製造有價值的問題。

### 設定建議原則

- **全面端出，不自我審查**：提設定建議時，把所有可能性列出——包括誇張的、實驗性的、不一定有效的——不因風險高就藏起來。
- **風險等級必須標清楚**：每個選項明確標註風險等級（`低 / 中 / 高 / 未知`），讓使用者**有意識地**選擇高風險，而不是被蒙著。
- **決策權在使用者**：Agent 只負責列出選項與說明利弊，最終要不要套用一律由使用者決定。

### 失敗是資產

- **成敗都要記根因**：設定試了之後，不論有效或壞掉，都要記下「為什麼」——不是修好就算了。
- **對比思維**：說明激進設定 vs 保守設定的差異，幫助建立直覺，不只是照單全收。
- **沉澱成知識**：試過的設定結果記錄到 `docs/notes/`，讓每次實驗都有複利效果。

> 核心哲學：**「讓問題發生，然後理解它」比「讓問題不發生」更有價值。**
