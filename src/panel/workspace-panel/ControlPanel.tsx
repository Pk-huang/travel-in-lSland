      ...
      <div className="space-y-3">
        {selectedDay.timelineSections.map((section) => (
          <div key={section.sectionId} className="rounded-xl border border-white/10 bg-black/15 p-3">
            <p className="mb-2 text-xs font-semibold tracking-wide text-white/70">{section.label}</p>
...
function TravelMarkersList({ travelMarkers }: { travelMarkers: ReturnType<typeof buildTravelMapMarkers> }) {
  return (
    <div className="rounded-xl border border-white/10 bg-black/15 p-3">
      <p className="mb-2 text-xs font-semibold tracking-wide text-white/70">地圖站點（{travelMarkers.length}）</p>
