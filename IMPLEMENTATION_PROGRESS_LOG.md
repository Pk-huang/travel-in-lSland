...
- 本輪重點：
  1. 以領域整理現有元件與資料夾：map / panel / providers / store / travel-plans / lib。
  2. 釐清 app、頁面容器、共享狀態與資料取得流程的責任邊界。
  3. 把跨元件共用邏輯收斂為單一真相來源，避免重複狀態與重複實作。
  4. 在不新增大功能的前提下，先建立可被新成員快速理解的結構規則。
- 目前狀態總結：
  - 已完成：基礎資料層、地圖與面板互動、旅行計畫資料流與部分 POI/地圖整合。
  - 目前瓶頸：功能已多，但責任分層仍有些混雜，後續擴充容易出現邏輯散落與狀態重複。
  - 本階段優先處理：架構清晰度、組件拆分、狀態與資料流整理。
- 暫緩項目：完整 E2E 測試、大規模 AI 功能擴張、LOD / 地形架構大改。
...
- 目前狀態總結：
  ...
  - 本階段優先處理：架構清晰度、組件拆分、狀態與資料流整理。
- 暫緩項目：完整 E2E 測試、大規模 AI 功能擴張、LOD / 地形架構大改。
>>>>>>> 36be839 (docs(plan): prioritize architecture refactor in phase 3)

## 2026-07-24 決議紀錄（UI / 地圖優先）

- 狀態：完成記錄
- 決議摘要：
	- 旅行計劃功能開發順序調整為「先 UI + 地圖互動，再進高風險智慧排程」。
	- 本輪優先交付：地圖站點順序標記、單日行程 Google Maps 導引、全部/單日切換、單日與全行程編輯能力。
	- 使用者定位列為本輪範圍，但需遵守瀏覽器地理定位授權流程。
	- 自動排程建議（依天數、開車時數、景點類型重排）保留，不納入本輪實作。
- 後續動作（依序）：
	1. 先做地圖站點順序標記（可視化驗收）。
	2. 再做單日行程 Google Maps 單站導引（座標優先、地名 fallback）。
	3. 再做左側「全部/單日」切換與清單渲染一致性。
	4. 最後串接拖曳編輯（單日優先，再到全行程）。
...
### 3-4 一週功能優先衝刺（Demo Ready Sprint）
- 狀態：進行中（2026-07-20 啟動）
- 目標：在 1 週內完成「景點與照片展示 + 旅行清單 CRUD + 輕量 AI」的可演示閉環。
- 範圍凍結（本週不做）：LOD 重啟、地形架構大改、完整深度 E2E 全覆蓋。
- 最小測試底線（每個功能小步都要過）：
	- `get_errors`
	- `corepack pnpm lint`
	- `curl -s -o /dev/null -w "home:%{http_code}\n" http://localhost:3000/`
	- 核心 smoke：fallback 不白屏、CRUD 主流程可走
- 每日 DoD：
	- Day 1-2：POI 清單/詳情 + 照片顯示 + 錯圖 fallback
	- Day 3-4：旅行清單 CRUD（新增/編輯/刪除/完成）+ 從 POI 一鍵加入
	- Day 5：輕量 AI 產生 3~5 筆建議 + 一鍵加入清單（去重）
	- Day 6：清單驅動優化（推薦排序、風險提示）+ UI 狀態一致化
	- Day 7：連續 Demo 彩排、修補高風險缺陷
- 驗收結果（完成條件）：
	- 可穩定展示「看景點/照片 -> 建立與管理清單 -> AI 補建議 -> 回填清單」
	- API 波動時不白屏，可連續 Demo
...
### 2026-07-20（Phase 3-4 衝刺策略調整）
- 今日完成：確認採「功能優先 + 最小測試防線」雙軌策略。
- 今日完成：主規格新增 3-4 一週衝刺，進度檔同步每日 DoD 與驗收條件。
- 今日決策：不採「功能全做完再補測試」，改為每個功能小步都跑最小測試底線。
- 今日阻塞：無（待進入 Day 1-2 的 POI/照片實作）。
- 下一步：啟動 3-4 Day 1-2，先完成 POI + 照片展示與 fallback。
